    ...
    axios
      ...
      .then((res) => {
        if (res.data.Status === 'Success') { 
          localStorage.setItem('token', res.data.token);// Stocke le token dans localStorage
          console.log('Rôle de l\'utilisateur:', res.data.user.role); // Affiche le rôle dans la console
          console.log("Cookies juste après connexion depuis le doc:", document.cookie); // Log des cookies du navigateur
          console.log("Token reçu depuis la réponse:", res.data.token); // Log du token
          navigate('/enter', { state: { userName: res.data.user.name } }); // Redirection après succès
        } else {
