...
function Login() {
  ...
  const handleSubmit = (event) => {
    ...
    axios.post(`${apiUrl}/api/auth/login`, values, { withCredentials: true })
      .then(res => {
        if (res.data.Status === "Success") {
          const token = res.data.token;
          localStorage.setItem('token', token);
          console.log('Rôle de l\'utilisateur:', res.data.user.role); // Affiche le rôle dans la console
      ...
      <div className='d-flex justify-content-center align-items-center flex-grow-1' style={{ backgroundColor: '#000235' }}>
        ...
        <div className='d-flex justify-content-between align-items-center mb-5'>
          ...
          <div className='row d-flex justify-content-center'>
            <div className='bg-white p-3 rounded col-12 col-md-8 col-lg-6 mb-4' style={{ maxWidth: '500px', maxHeight: '75vh', overflowY: 'auto' }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <button type='submit' className='btn btn-dark w-100 rounded-1'>Connexion</button>
                <Link to="/" className='btn btn-dark w-100 rounded-1 mt-2'>Accueil</Link>
                <p>Vous acceptez nos conditions et politiques</p>
                <Link to="/register" className='btn btn-dark border w-100 rounded-1'>S'inscrire</Link>
              </form>
            </div>
          </div>
        </div>
      </div>
      <footer className="text-white mt-auto" style={{ backgroundColor: 'rgba(7, 1, 8, 0.96)', padding: '20px 0' }}>
        <div className="container">
          <div className="row">
